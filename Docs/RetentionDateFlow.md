...
- 以開關控制 **搬移完成後刪除來源資料** (`Form.IsPhysicalDeleteEnabled`)，預設開啟，以維持舊有「搬移後同步刪除」邏輯；關閉時僅寫入歷史庫不刪除來源。
...
- `IsPhysicalDeleteEnabled` 為 true 時，搬移批次完成後會呼叫 `DeleteByPrimaryKeysAsync` 拆批刪除來源庫；為 false 則僅寫入歷史庫並記錄「保留來源」訊息，避免誤刪資料。
